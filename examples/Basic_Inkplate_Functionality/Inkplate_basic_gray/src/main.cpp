/*
   ...
   Inkplate will be used in gray mode which is 3 bit, so you can have up to 8 different colors
   (black, 6 gray colors and white) Color is represented by number, where number 0 means black and
   number 7 means white, while everything in between are shades of gray.
...
Inkplate
    display(DisplayMode::INKPLATE_3BIT); // Create object on Inkplate library and set library to
                                         // work in gray mode (3-bit) Other option is BW mode, which
                                         // is demonstrated in next example "Inkplate_basic_BW"

#define DELAY_MS                                                                                   \
  5000 // Delay in milliseconds between screen refresh. Refreshing e-paper screens more often than
       // 5s is not recommended Want to refresh faster? Use partial update! Find example in
...
void mainTask(void *params) {
  ...
  display.clearDisplay(); // Clear any data that may have been in (software) frame buffer.
                          //(NOTE! This does not clean image on screen, it only clears it in the
                          // frame buffer inside
                          // ESP32).
  ...
  for (;;) {
    ...
    // Example will demostrate funcionality one by one. You always first set everything in the frame
    // buffer and afterwards you show it on the screen using display.display().
    ...
    display.drawPixel(100, 50,
                      0); // Draw one black pixel at X = 100, Y = 50 position in 0 (BLACK) color
    display.drawRect(95, 145, 10, 10, 0);
    displayCurrentAction("Drawing a pixel"); // Function which writes small text at bottom left
                                             // indicating what's currently done NOTE: you do not
                                             // need displayCurrentAction function to use Inkplate!
    display.display(); // Send image to display. You need to call this one each time you want to
                       // transfer frame buffer to the screen.
    ESP::delay(5000);  // Wait a little bit
    ...
    display.drawLine(0, 0, w - 1, h - 1,
                     0); // All of those drawing fuctions originate from Adafruit GFX library, so
                         // maybe you are already familiar
    display.drawLine(w - 1, 0, 0, h - 1,
                     0); // with those. Arguments are: start X, start Y, ending X, ending Y, color.
    ...
    display.clearDisplay();
    for (int i = 0; i < 100; i++) {
      display.drawLine(random(0, w - 1), random(0, h - 1), random(0, w - 1), random(0, h - 1),
                       random(0, 7));
    }
    ...
    display.clearDisplay();
    for (int i = 0; i < 100; i++) {
      display.drawThickLine(random(0, w - 1), random(0, h - 1), random(0, w - 1), random(0, h - 1),
                            random(0, 7), (float)random(1, 20));
    }
    ...
    for (int i = 0; i < 100; i++) {
      ...
      display.drawGradientLine(random(0, w - 1), random(0, h - 1), random(0, w - 1),
                               random(0, h - 1), startColor, endColor, (float)random(1, 20));
    }
    ...
    display.clearDisplay();
    display.drawFastHLine(100, 100, h - 200,
    ...
    display.clearDisplay();
    display.drawFastVLine(100, 100, w - 200,
                          0); // Arguments are: starting X, starting Y, length, color
    ...
    display.clearDisplay();
    display.drawRect(200, 200, w / 2, h / 2,
    ...
    display.clearDisplay();
    display.fillRect(200, 200, w / 2, h / 2,
                     4); // Arguments are: start X, start Y, size X, size Y, color
    displayCurrentAction("Drawing gray rectangle");
    display.display();
    ESP::delay(5000);

    // Draw filled random colored rectangles on random location, size of 30x30 pixels in radnom
    // color
    display.clearDisplay();
    for (int i = 0; i < 50; i++) {
    ...
    display.clearDisplay();
    display.drawRoundRect(200, 200, w / 2, h / 2, 10,
                          0); // Arguments are: start X, start Y, size X, size Y, radius, color
    displayCurrentAction("Drawing rectangle with rounded edges");
    display.display();
    ESP::delay(5000);

    // Draw rounded rectangles on random location, size 100x150 pixels, radius of 5 pixels in radnom
    // color
    display.clearDisplay();
    for (int i = 0; i < 50; i++) {
    ...
    // Draw filled random colored rectangle at X = 200, Y = 200, size of 400x300 pixels and radius
    // of 10 pixels
    display.clearDisplay();
    display.fillRoundRect(200, 200, w / 2, h / 2, 10,
                          0); // Arguments are: start X, start Y, size X, size Y, radius, color
    displayCurrentAction("Drawing filled rectangle with rounded edges");
    display.display();
    ESP::delay(5000);

    // Draw filled random colored rectangle on random location, size of 30x30 pixels, radius of 3
    ...
    display.fillTriangle(300, 350, 500, 350, 400, 150,
                         0); // Arguments are: X1, Y1, X2, Y2, X3, Y3, color
    displayCurrentAction("Drawing filled triangle inside exsisting one");
    display.display();
    ESP::delay(5000);

    // Display some grayscale image on screen. We are going to display e-radionica logo on display
    // at location X = 100, Y = 100 Image size is 500x332 pixels.
    display.clearDisplay();
    ...
    // is black)
    display.setTextColor(7, 0); // First argument is text color, while second argument is background
                                // color. In greyscale,
