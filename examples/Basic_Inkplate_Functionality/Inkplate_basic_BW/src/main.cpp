...
void mainTask(void *params) {
  ...
  for (;;) {
    ...
    display.drawPixel(100, 50, BLACK); // Draw one black pixel at X = 100, Y = 50 position in BLACK
    ...
    display.drawLine(0, 0, w - 1, h - 1,
                     BLACK); // All of those drawing fuctions originate from Adafruit GFX library,
                             // so maybe you are already familiar
    display.drawLine(
        w - 1, 0, 0, h - 1,
    ...
    display.clearDisplay();
    display.drawFastHLine(100, 100, w - 200,
    ...
    display.clearDisplay();
    display.drawFastVLine(100, 100, h - 200,
                          BLACK); // Arguments are: starting X, starting Y, length, color
    ...
    display.clearDisplay();
    display.fillRect(200, 200, 400, 300,
                     BLACK); // Arguments are: start X, start Y, size X, size Y, color
    ...
    // Display some bitmap on screen. We are going to display e-radionica logo on display at
    // location X = 200, Y = 200 Image is 576x100 pixels and we want to every pixel of this bitmap
    // to be black.
    display.clearDisplay();
    display.drawImage(
        logo, (w / 2) - (576 / 2), (h / 2) - 50, 576, 100,
