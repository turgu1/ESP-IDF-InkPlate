/*
   The purpose of this example is to show how to use the ESP-IDF-InkPlate library without using
   Platformio SDK It will display a message "Hello World" in the center of the screen and then exit
   the program NOTE: Partial update is only available on 1 Bit mode (BW) and it is not recommended
   to use it on first refresh after power up. It is recommended to do a full refresh every 5-10
   partial refresh to maintain good picture quality.
...
void mainTask(void *param) {
  ...
  display.getTextBounds(message, current_x, current_y, &boundary_x, &boundary_y, &message_width,
                        &message_height);
