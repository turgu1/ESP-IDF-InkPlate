...
double modulusComplexSqr(struct complex *z) { return z->re * z->re + z->im * z->im; }
    ...
    for (int j = 0; j < display.height(); ++j) {
      for (int i = 0; i < display.width(); ++i)
        display.drawPixel(i, j,
                          colorAt(xFrom + (double)i * (xTo - xFrom) / 800.0,
                                  yFrom + (double)j * (yTo - yFrom) / 600.0));
      // for whole set:
      // display.drawPixel(i, j, colorAt(-2.0 + (3.0 * (double)i / 800.0), -1.0 + 2.0 * (double)j /
      // 600.0));
      ESP_LOGI(TAG, "%d", j);
    }
    display.display();
...
void app_main() {
  ...
  xTaskCreate(mandelbrot_task, "mainTask", STACK_SIZE, (void *)1, configMAX_PRIORITIES - 1,
              &xHandle);
