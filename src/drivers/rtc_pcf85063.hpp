...
public:
  enum class CalibrationMode : uint8_t {
    EVERY_TWO_HOURS = 0x00, EVERY_FOUR_MINUTES = 0x01
  };
  ...
  enum class CAPACITOR : uint8_t {
    SEL_7PF = 0, SEL_12_5PF = 1
  };

private:
  static constexpr char const *TAG = "RTC";
  const uint8_t rtc_address;
  WireDevice *rtc_device;
  enum class Reg : uint8_t {
    CTRL1 = 0x00, CTRL2 = 0x01, OFFSET = 0x02, RAM = 0x03, SEC = 0x04, MIN = 0x05, HOUR = 0x06,
    DAY = 0x07, WEEKDAY = 0x08, MONTH = 0x09, YEAR = 0x0A, SEC_ALARM = 0x0B, MIN_ALARM = 0x0C,
    HOUR_ALARM = 0x0D, DAY_ALARM = 0x0E, WEEKDAY_ALARM = 0x0F, TIMER_VAL = 0x10, TIMER_MODE = 0x11
  };
...
public:
  ...
  void set_date_time(uint16_t y, uint8_t m, uint8_t d, uint8_t h, uint8_t mm, uint8_t s,
                     WeekDay wd);

  void set_date_time(const time_t *t);

  void get_date_time(uint16_t &y, uint8_t &m, uint8_t &d, uint8_t &h, uint8_t &mm, uint8_t &s,
                     WeekDay &wd);
