  ...
  bool TouchScreen::software_reset() {
    ...
    touch_device->write(reset_cmd, sizeof(reset_cmd));
    ...
    if (timeout <= 0) {
      ...
      uint8_t answer[4];
      if (touch_device->read(answer, sizeof(answer))) {
  ...
  void TouchScreen::retrieve_resolution() {
    ...
    touch_device->cmd_read(cmd_get_x_resolution, sizeof(cmd_get_x_resolution), answer,
                           sizeof(answer));
    x_resolution = ((answer[3] & 0xf0) << 4) + answer[2];

    touch_device->cmd_read(cmd_get_y_resolution, sizeof(cmd_get_y_resolution), answer,
                           sizeof(answer));
  ...
  uint8_t TouchScreen::get_position(TouchPositions &x_positions, TouchPositions &y_positions) {
    ...
    Wire::enter();
    touch_device->read(raw, sizeof(raw));
    Wire::leave();
  ...
  void TouchScreen::set_power_state(bool on_state) {
    ...
    Wire::enter();
    touch_device->write(power_state_reg, sizeof(power_state_reg));
    Wire::leave();
  }
  ...
  bool TouchScreen::get_power_state() {
    ...
    Wire::enter();
    touch_device->cmd_read(power_state_cmd, sizeof(power_state_cmd), data, sizeof(data));
    Wire::leave();
