  ...
  bool FrontLight::setup() {
    ESP_LOGD(TAG, "Initializing...");

    wire.setup();
    Wire::enter();

    light_device = new WireDevice(FRONTLIGHT_ADDRESS);
    if ((light_device == nullptr) || !light_device->is_initialized()) return false;
  ...
  void FrontLight::set_level(uint8_t level) {
    if (level > 63) return;
    ...
    Wire::enter();
    light_device->cmd_write(0, static_cast<uint8_t>(63 - (level & 0b00111111)));

    Wire::leave();
  }

  void FrontLight::enable() {
    if (!enabled) {
      ESP_LOGD(TAG, "Enable...");
      Wire::enter();
