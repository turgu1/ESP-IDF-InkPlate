...
*/
  ...
  bool IOExpander::setup() {
    ESP_LOGD(TAG, "Initializing...");

    mcp_device = new WireDevice(mcp_address);
    present    = (mcp_device != nullptr) && mcp_device->is_initialized();

    ESP_LOGD(TAG, "MCP at address 0x%X has%s been detected", mcp_address, present ? "" : " NOT");
  ...
  void IOExpander::read_all_registers() {
    if (!check_presence()) return;

    mcp_device->cmd_read(0x00, registers.data(), registers.size());
  }

  void IOExpander::read_registers(Reg first_reg, uint8_t count) {
    if (!check_presence()) return;

    mcp_device->cmd_read(static_cast<int8_t>(first_reg), &registers[first_reg], count);
  }

  uint8_t IOExpander::read_register(Reg reg) {
    if (!check_presence()) return 0;

    registers[reg] = mcp_device->cmd_read(static_cast<int8_t>(reg));

    return registers[reg];
  }

  void IOExpander::update_all_registers() {
    if (!check_presence()) return;

    mcp_device->cmd_write(0x00, registers.data(), registers.size());
  }

  void IOExpander::update_register(Reg reg, uint8_t value) {
    if (!check_presence()) return;

    mcp_device->cmd_write(static_cast<uint8_t>(reg), value);
  }

  void IOExpander::update_registers(Reg first_reg, uint8_t count) {
    if (!check_presence()) return;

    mcp_device->cmd_write(static_cast<uint8_t>(first_reg), &registers[first_reg], count);
  }

  // HIGH LEVEL:

  void IOExpander::set_direction(Pin pin, PinMode mode) {
  ...
  void IOExpander::digital_write(Pin pin, SignalLevel state) {
    uint8_t port = (static_cast<uint8_t>(pin) >> 3) & 1;
    uint8_t p    = static_cast<uint8_t>(pin) & 7;
  ...
  IOExpander::SignalLevel IOExpander::digital_read(Pin pin) {
    uint8_t port = (static_cast<uint8_t>(pin) >> 3) & 1;
    uint8_t p    = static_cast<uint8_t>(pin) & 7;
    uint8_t r    = read_register((Reg)(static_cast<int8_t>(Reg::GPIOA) + port));

    return (r & (1 << p)) ? SignalLevel::HIGH : SignalLevel::LOW;
  }

  void IOExpander::set_int_output(IntPort intPort, bool mirroring, bool openDrain,
                                  SignalLevel polarity) {
  ...
  void IOExpander::set_int_pin(Pin pin, IntMode mode) {
    uint8_t port = (static_cast<uint8_t>(pin) >> 3) & 1;
    uint8_t p    = static_cast<uint8_t>(pin) & 7;
  ...
  void IOExpander::remove_int_pin(Pin pin) {
    uint8_t port = (static_cast<uint8_t>(pin) >> 3) & 1;
    uint8_t p    = static_cast<uint8_t>(pin) & 7;
    registers[R(Reg::GPINTENA, port)] &= ~(1 << p);
