...
*/
  ...
  bool IOExpander::check_presence() {
    if (!present)
      ESP_LOGE(TAG, "The PCAL6416 at address 0x%X has not been detected.", pcal_address);
    return present;
  }
  ...
  bool IOExpander::setup() {
    ...
    pcal_device = new WireDevice(pcal_address);
    present     = (pcal_device != nullptr) && pcal_device->is_initialized();
  ...
  void IOExpander::read_all_registers() {
    ...
    pcal_device->cmd_read(0x00, registers.data(), registers.size());
  }

  void IOExpander::read_registers(Reg first_reg, uint8_t count) {
    if (!check_presence()) return;

    pcal_device->cmd_read(reg_addresses[static_cast<int8_t>(first_reg)], &registers[first_reg],
                          count);
  }
  ...
  uint8_t IOExpander::read_register(Reg reg) {
    ...
    registers[reg] = pcal_device->cmd_read(reg_addresses[static_cast<int8_t>(reg)]);
  ...
  void IOExpander::update_all_registers() {
    ...
    pcal_device->cmd_write(0x00, registers.data(), registers.size());
  }

  void IOExpander::update_register(Reg reg, uint8_t value) {
    if (!check_presence()) return;

    pcal_device->cmd_write(reg_addresses[static_cast<int8_t>(reg)], value);
  }

  void IOExpander::update_registers(Reg first_reg, uint8_t count) {
    if (!check_presence()) return;

    pcal_device->cmd_write(reg_addresses[static_cast<int8_t>(first_reg)], &registers[first_reg],
                           count);
  }
