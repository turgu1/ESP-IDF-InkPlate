 ...
 */
  ...
  const uint8_t EInk6FLICK::LUT2[16] = { 0xAA, 0xA9, 0xA6, 0xA5, 0x9A, 0x99, 0x96, 0x95,
                                         0x6A, 0x69, 0x66, 0x65, 0x5A, 0x59, 0x56, 0x55 };

  const uint8_t EInk6FLICK::LUTW[16] = { 0xFF, 0xFE, 0xFB, 0xFA, 0xEF, 0xEE, 0xEB, 0xEA,
                                         0xBF, 0xBE, 0xBB, 0xBA, 0xAF, 0xAE, 0xAB, 0xAA };

  const uint8_t EInk6FLICK::LUTB[16] = { 0xFF, 0xFD, 0xF7, 0xF5, 0xDF, 0xDD, 0xD7, 0xD5,
                                         0x7F, 0x7D, 0x77, 0x75, 0x5F, 0x5D, 0x57, 0x55 };

  bool EInk6FLICK::setup() {
  ...
  void EInk6FLICK::update(FrameBuffer1Bit &frame_buffer) {
    ...
    for (int k = 0; k < 1; ++k) {
      ...
      for (int i = 0; i < HEIGHT; ++i) {
        ...
        for (int n = 0; n < (WIDTH / 4); n += 4) {
          ...
          *buff++ = LUT2[(dram2 >> 4) & 0x0F]; // i + 2;
          *buff++ = LUT2[dram2 & 0x0F];        // i + 3;
          *buff++ = LUT2[(dram1 >> 4) & 0x0F]; // i;
          *buff++ = LUT2[dram1 & 0x0F];        // i + 1;
        }
  ...
  void EInk6FLICK::update(FrameBuffer3Bit &frame_buffer) {
    ...
    for (int k = 0, kk = 0; k < 9; ++k, kk += 256) {
      ...
      for (int i = 0; i < HEIGHT; ++i) {
        ...
        for (int j = 0; j < (WIDTH / 4); j += 4) {
          ...
          dp -= 2;
          line_buffer[j] = (GLUT2[kk + dp[1]] | GLUT[kk + dp[0]]);
          dp -= 2;
          line_buffer[j + 1] = (GLUT2[kk + dp[1]] | GLUT[kk + dp[0]]);
          dp -= 2;
        }
  ...
  void EInk6FLICK::partial_update(FrameBuffer1Bit &frame_buffer, bool force) {
    ...
    uint8_t *ptr   = &p_buffer[BITMAP_SIZE_1BIT * 2];
    uint32_t pos = BITMAP_SIZE_1BIT;
    ...
    for (int i = 0; i < HEIGHT; ++i) {
      for (int j = 0; j < LINE_SIZE_1BIT; ++j) {
        ...
        *--ptr = LUTW[diffw >> 4] & (LUTB[diffb >> 4]);
        *--ptr = LUTW[diffw & 0x0F] & (LUTB[diffb & 0x0F]);
      }
    }
  ...
  void EInk6FLICK::clean(PixelState pixel_state, uint8_t repeat_count) {
    if (!turn_on()) { return; }
