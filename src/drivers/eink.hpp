...
class EInk {
  public:
    ...
    #if INKPLATE_5_V2 || INKPLATE_6_V2 || INKPLATE_6FLICK || INKPLATE_10_V2 || INKPLATE_6PLUS_V2 ||    \
    INKPLATE_6PLUS
      static const gpio_num_t WAKE_UP_PIN = GPIO_NUM_36;
    #endif
  ...
  protected:
      ...
      EInk(IOExpander &io_expander, const int screen_width)
        : io_expander_int(io_expander), i2s_comms(I2SComms((screen_width / 4) + 16)),
        panel_state(PanelState::OFF), initialized(false), partial_allowed(false) {}
    #else
      EInk(IOExpander &io_expander)
        : io_expander_int(io_expander), panel_state(PanelState::OFF), initialized(false),
    ...
    inline void wakeup_clear() {
      io_expander_int.digital_write(WAKEUP, IOExpander::SignalLevel::LOW);
    }
