  ...
  static void IRAM_ATTR touchscreen_isr(void *value) {
  ...
  bool TouchScreen::setup(bool power_on, ISRHandlerPtr isr_handler) {
    ...
    touch_device = new WireDevice(TOUCHSCREEN_ADDRESS);
    if ((touch_device == nullptr) || !touch_device->is_initialized()) {
      ESP_LOGE(TAG, "Setup error: %s",
               touch_device == nullptr ? "NULL Device!" : "Not initialized!");
      return false;
    }
    ...
    if (power_on) {
      ...
      if (ping(5) && send_command(SOFT_RESET_MODE) && get_boot_loader_data(boot_loader_data) &&
          exit_boot_loader_mode() && get_sys_info(sys_info_data) &&
          set_sys_info_regs(sys_info_data) && send_command(OPERATE_MODE) &&
          touch_device->cmd_write(DETECTION_DISTANCE, distance_default)) {
        ...
        gpio_isr_handler_add(INTERRUPT_PIN, touchscreen_isr, (void *)INTERRUPT_PIN);
  ...
  // ok
  void TouchScreen::set_app_isr_handler(ISRHandlerPtr isr_handler) {
  ...
  // ok
  void TouchScreen::hardware_reset() {
  ...
  bool TouchScreen::software_reset() {
  ...
  uint8_t TouchScreen::get_position(TouchPositions &x_positions, TouchPositions &y_positions) {
    ...
    bool res = touch_device->cmd_read(BASE_ADDR, raw, sizeof(raw));
    Wire::leave();
  ...
  bool TouchScreen::get_power_state() {
    bool power_is_on =
        io_expander.digital_read(TOUCHSCREEN_ENABLE) == IOExpander::SignalLevel::HIGH;

    return power_is_on;
  }

  bool TouchScreen::send_command(uint8_t command) {
    bool res = touch_device->cmd_write(BASE_ADDR, command);
  ...
  bool TouchScreen::is_screen_touched() { return touchscreen_interrupt_happened; }

  bool TouchScreen::get_boot_loader_data(BootLoaderData &bl_data) {
    if (!touch_device->cmd_read(BASE_ADDR, (uint8_t *)&bl_data, sizeof(BootLoaderData))) {
  ...
  bool TouchScreen::exit_boot_loader_mode() {
    ...
    touch_device->cmd_write(BASE_ADDR, command_data, sizeof(command_data));
  ...
  bool TouchScreen::get_sys_info(SysInfoData &sys_info_data) {
    ...
    // Read the registers.
    if (!touch_device->cmd_read(BASE_ADDR, sys_info_array, sizeof(sys_info_array))) {
  ...
  bool TouchScreen::set_sys_info_regs(SysInfoData &sys_info_data) {
    ...
    uint8_t regs[] = {ACT_INTERVAL_DEFAULT, TOUCH_TIMEOUT_DEFAULT, LOW_POWER_INTERVAL_DEFAULT};

    // Send the registers to the I2C. Check if failed. If failed, return false.
    if (!touch_device->cmd_write(REG_ACT_INTERVAL, regs, 3)) {
  ...
  void TouchScreen::handshake() {
    // Read the hst_mode register (address 0x00).
    uint8_t host_mode_reg;
    host_mode_reg = touch_device->cmd_read(BASE_ADDR);
    host_mode_reg ^= 0x80;
    touch_device->cmd_write(BASE_ADDR, host_mode_reg);
  }

  bool TouchScreen::ping(int retries) {
