...
uint8_t PressKeys::read_all_keys() {
  Wire::enter();
...
uint8_t PressKeys::read_key(Key key) {
  Wire::enter();
  IOExpander::SignalLevel value =
      io_expander.digital_read((IOExpander::Pin)((static_cast<uint8_t>(key)) + 10)); // Not clean
  Wire::leave();
