...
static void sta_event_handler(void *arg, esp_event_base_t event_base, int32_t event_id,
                              void *event_data) {
  ESP_LOGI(TAG, "STA Event, Base: %08x, Event: %" PRIi32 ".", (unsigned int)event_base, event_id);
...
bool NetworkClient::joinAP(const char *ssid, const char *pass) {
  ...
  if (bits & WIFI_CONNECTED_BIT) {
    ...
    ESP_LOGI(TAG, "Failed to connect to SSID: %s, password: %s", wifi_config.sta.ssid,
             wifi_config.sta.password);
  } else {
...
void NetworkClient::disconnect() {
  if (connected) {
    ...
    ESP_ERROR_CHECK(
        esp_event_handler_unregister(ESP_EVENT_ANY_BASE, ESP_EVENT_ANY_ID, &sta_event_handler));
    ESP_ERROR_CHECK(esp_event_loop_delete_default());
...
uint8_t *NetworkClient::downloadFile(const char *url, int32_t *defaultLen) {
  ...
  if (err == ESP_OK) {
    ESP_LOGI(TAG, "Status = %d, content_length = %" PRIi64, esp_http_client_get_status_code(client),
