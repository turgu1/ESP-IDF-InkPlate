...
class Wire : NonCopyable {
  ...
  public:
    ...
    inline void init_device(uint8_t addr, i2c_master_dev_handle_t *dev_handle) {
      ...
      dev_cfg.scl_wait_us             = DEFAULT_TIMEOUT;
    ...
    inline bool flush() { return i2c_master_bus_wait_all_done(master_bus_handle, -1) == ESP_OK; }
};
...
class WireDevice {
  ...
  public:
    ...
    #if DEBUG_WIRE
      void show_data(const char *from, const uint8_t *data, int length, const uint8_t *cmd = nullptr,
                     int cmd_length = 0) {
    ...
    inline bool write(const uint8_t *data, int length, int timeout = DEFAULT_TIMEOUT) {
      #if DEBUG_WIRE
    ...
    bool cmd_write(uint8_t cmd, const uint8_t *data, int length, int timeout = DEFAULT_TIMEOUT) {
      bool     result;
    ...
    inline bool cmd_write(uint8_t cmd, int timeout = DEFAULT_TIMEOUT) {
      #if DEBUG_WIRE
    ...
    inline bool cmd_write(uint8_t cmd, uint8_t data, int timeout = DEFAULT_TIMEOUT) {
      bool    result;
    ...
    inline bool read(uint8_t *data, int length, int timeout = DEFAULT_TIMEOUT) {
      ...
      bool      result      = status == ESP_OK;
        ...
        if (result) {
          ...
          ESP_LOGW(TAG, "No answer for device %02" PRIx8 ": error: %s", address,
    ...
    inline bool cmd_read(uint8_t cmd, uint8_t *data, int length, int timeout = DEFAULT_TIMEOUT) {
    ...
    inline uint8_t cmd_read(uint8_t cmd, int timeout = DEFAULT_TIMEOUT) {
      uint8_t data = 0;
      ...
      #else
        i2c_master_transmit_receive(dev_handle, &cmd, 1, &data, 1, DEFAULT_TIMEOUT);
      #endif
