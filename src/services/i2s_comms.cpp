  ...
  void I2SComms::init(uint8_t clockDivider) {
    ...
    i2s_dev->clkm_conf.clkm_div_num = clockDivider;

    // FIFO buffer setup. Byte packing for FIFO: 0A0B_0B0C = 0, 0A0B_0C0D = 1, 0A00_0B00 = 3. Use
    // dual mono single data
    i2s_dev->fifo_conf.val                  = 0;
    i2s_dev->fifo_conf.rx_fifo_mod_force_en = 1;
    i2s_dev->fifo_conf.tx_fifo_mod_force_en = 1;
    // byte packing 0A0B_0B0C = 0, 0A0B_0C0D = 1, 0A00_0B00 = 3. Use dual mono single data
  ...
  void I2SComms::set_pin(uint32_t pin, uint32_t function, uint32_t inv) {
    ...
    ESP_REG(io_mux[pin]) = ((3 << FUN_DRV_S) | (2 << MCU_SEL_S));
  }
